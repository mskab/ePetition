...
def create(db: Session, decision_maker: DecisionMakerCreate):
    ...
    db_decision_maker = DecisionMaker(naming=decision_maker.naming,
...
def is_decision_maker_exist_by_email(db: Session, email: str):
    db_decision_maker = db.query(DecisionMaker).filter(
        DecisionMaker.email == email).first()
    if db_decision_maker:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Decision maker already exists")


def get_by_id(db: Session, decision_maker_id: int):
    decision_maker = db.query(DecisionMaker).filter(
        DecisionMaker.id == decision_maker_id).first()
